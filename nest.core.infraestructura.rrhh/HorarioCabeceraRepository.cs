...
namespace nest.core.infraestructura.rrhh
{
    public class HorarioCabeceraRepository : CrudRepositoryBase<HorarioCabecera, int>, IHorarioRepository
    {
        ...
        public async Task<LoadResult> ObtenerFilterActivos(DataSourceLoadOptionsBase options, CancellationToken cancellationToken) => await DataSourceLoader.LoadAsync(Query().Where(h => h.Activo), options, cancellationToken);
    }
}
