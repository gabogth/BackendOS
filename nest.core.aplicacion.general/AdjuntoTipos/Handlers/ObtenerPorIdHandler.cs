...
{
    internal class ObtenerPorIdHandler : IRequestHandler<ObtenerPorIdQuery, AdjuntoTipo>
    {
        private readonly IAdjuntoTipoRepository repository;
        private readonly ILogger<ObtenerPorIdHandler> logger;

        public ObtenerPorIdHandler(IAdjuntoTipoRepository repository, ILogger<ObtenerPorIdHandler> logger)
        {
            this.repository = repository;
            this.logger = logger;
        }

        public async Task<AdjuntoTipo> Handle(ObtenerPorIdQuery request, CancellationToken cancellationToken)
        {
            try
            {
