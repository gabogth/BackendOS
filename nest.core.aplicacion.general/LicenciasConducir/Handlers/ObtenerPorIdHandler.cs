...
{
    public sealed class ObtenerPorIdHandler : IRequestHandler<ObtenerPorIdQuery, LicenciaConducir>
    {
        private readonly ILicenciaConducirRepository repository;
        private readonly ILogger<ObtenerPorIdHandler> logger;

        public ObtenerPorIdHandler(ILicenciaConducirRepository repository, ILogger<ObtenerPorIdHandler> logger)
        {
            this.repository = repository;
            this.logger = logger;
        }

        public async Task<LicenciaConducir> Handle(ObtenerPorIdQuery request, CancellationToken cancellationToken)
        {
            try
            {
